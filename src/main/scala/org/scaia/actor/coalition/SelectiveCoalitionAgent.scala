...
class SelectiveCoalitionAgent(a: Activity, approximation: Boolean, rule: SocialRule) extends  CoalitionAgent(a: Activity, approximation: Boolean, rule: SocialRule) {
  ...
  override def disposing(): Receive = {
    case Propose(i) => {
      ...
      if (g.isEmpty) {
        if (debug) log.debug("Since the current group of " + a.name + " is empty " + lastProposer + " is assigned to the activity " + a.name)
        g += lastProposer
        sender ! Accept
        ...
        if (a.c > g.size) {
          if (debug) log.debug("The quota of " + a.name + " is not reached")
